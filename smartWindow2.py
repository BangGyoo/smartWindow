...
def SetWindow(sensor_flags,output_flags) :
	if output_flags[0] == True :
		os.system("python ./inner/motor_cw_ccw.py cw 23")
	elif output_flags[1] == True :
		os.system("python ./inner/motor_cw_ccw.py ccw 23")
		...
		os.system("python ./moter_on_off.py cw 23")
	...
	if output_flags[3] == True :
		if sensor_flags[3] == True :
			os.system("python ./inner/film.py on")
		else :
			os.system("python ./inner/film.py off")
...
while True :
	...
	if motion_flag == 1 :
		...
		output_flag[1] = True
	###########set user configure############################
	user_config_file = open("user_config.txt",r)
	...
	WHO = float(os.system("python ./outer/dht11.py 20"))
	WHI = float(os.system("python ./inner/dht11.py 20"))

	if abs(WHI - WHO) >= 0.1 :
		if WHI <= 0.4 || WHI >= 0.6 :
			WHG = WHI - WHO
	if (W1 * weighDust + W2 * WHG) > 1 :
	...
	WTI = float(os.system("python ./inner/dht11.py 20"))
	WTG = WTI - WTO
